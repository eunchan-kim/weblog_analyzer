...
def temp(filename, suspicious_log_id, num_list):
  connection = pymongo.MongoClient("mongodb://localhost")
  db = connection.test
  people = db.people
  cursor = people.find({"_id":ObjectId(suspicious_log_id)}) 
  binary_string = cursor[0]["list"]
  suspicious_log = json.loads(zlib.decompress(str(binary_string)))

  abnormal_log = get_log_from_number(filename, num_list)

  for log in suspicious_log:
    if log in abnormal_log:
      abnormal_log.remove(log)
